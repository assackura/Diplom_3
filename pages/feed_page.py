...
class FeedPage(BasePage):
    ...
    def open_modal_first_order(self):
        self.click_element(FeedPageLocators.ORDER_FEED_LINK, FeedPageLocators.FIRST_ORDER_IN_LIST_ORDERS)
        return self.wait_and_find_element(FeedPageLocators.SECTION_MODAL_OPENED)

    @allure.step('Создаем новый заказ')
    def create_new_order(self):
        self.click_element(FeedPageLocators.CONSTRUCTOR_LINK)
        self.create_order(FeedPageLocators.ING_BUN)
        number = ""
        while(True):
            number = self.wait_and_find_element(FeedPageLocators.ID_ORDER).text
            if number != "9999":
                break
        while(True):
            if len(number) < 7:
                ...
                break
        self.click_element(FeedPageLocators.CLOSE_BTN_MODAL_WINDOW)
        return number

    @allure.step('Получаем номер заказа в истории заказов')
    def get_order_number_from_history_orders(self):
        self.create_new_order()
        self.click_element(FeedPageLocators.PERSONAL_AREA_LINK, FeedPageLocators.HISTORY_ORDERS_LINK)
        return self.wait_and_find_element(FeedPageLocators.ORDER_NUMBER).text

    @allure.step('Получаем список заказов в ленте заказов')
    def get_list_orders_from_order_feed(self):
        self.click_element(FeedPageLocators.ORDER_FEED_LINK)
        list_orders = self.wait_and_find_elements(FeedPageLocators.ORDER_NUMBER_LOCATOR)
        return_list = []
